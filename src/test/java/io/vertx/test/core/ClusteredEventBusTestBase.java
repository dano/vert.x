...
public class ClusteredEventBusTestBase extends EventBusTestBase {
  ...
  protected <T, R> void testSendWildcard(T val, R received, Consumer<T> consumer, DeliveryOptions options) {
    ...
    MessageConsumer<T> reg = vertices[1].eventBus().<T>consumer(ADDRESS1, true).handler((Message<T> msg) -> {
      if (consumer == null) {
        assertEquals(received, msg.body());
        if (options != null) {
          assertNotNull(msg.headers());
    ...
    reg.completionHandler(ar -> {
      assertTrue(ar.succeeded());
      if (options == null) {
        vertices[0].eventBus().send(ADDRESS1 + ADDRESS2, val);
      } else {
  ...
  protected <T, R> void testReplyWildcard(T val, R received, Consumer<R> consumer, DeliveryOptions options) {
    ...
    String str = TestUtils.randomUnicodeString(1000);
    MessageConsumer<?> reg = vertices[1].eventBus().consumer(ADDRESS1, true).handler(msg -> {
      assertEquals(str, msg.body());
      if (options == null) {
        msg.reply(val);
      } else {
    ...
    reg.completionHandler(ar -> {
      assertTrue(ar.succeeded());
      vertices[0].eventBus().send(ADDRESS1 + ADDRESS2, str, onSuccess((Message<R> reply) -> {
        if (consumer == null) {
          assertEquals(received, reply.body());
          if (options != null && options.getHeaders() != null) {
            assertNotNull(reply.headers());
  ...
  protected <T> void testPublishWildcard(T val, Consumer<T> consumer) {
    ...
    MessageConsumer reg = vertices[2].eventBus().<T>consumer(ADDRESS1, true).handler(new MyHandler());
    reg.completionHandler(new MyRegisterHandler());
